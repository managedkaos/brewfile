...
def get_git_diff(compare_branch="main"):
...
2. If needed, includes a more detailed description after a blank line
3. Uses the imperative mood (e.g., "Add feature" not "Added feature")
4. Focuses on what changes and why, not how
...
Commit Message:"""
    ...
    try:
        ...
        truncated_diff = diff_text[:max_diff_length] + "..." if len(diff_text) > max_diff_length else diff_text
...
def main():
    parser = argparse.ArgumentParser(description="Generate commit messages from git diffs using Ollama")
    parser.add_argument("--branch", "-b", default="main",
                        help="Branch to compare against (default: main)")
    parser.add_argument("--model", "-m", default="phi4:latest",
